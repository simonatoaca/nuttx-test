...
#ifdef CONFIG_BME680_ENABLE_FLOAT_COMP
  float t_fine;
#else
  int32_t t_fine;
#endif /* CONFIG_BME680_ENABLE_FLOAT_COMP */
};
...
static int bme680_set_mode(FAR struct bme680_dev_s *priv, uint8_t mode)
{
  ...
  uint8_t power_mode = regval & BME680_MODE_MSK;
...
static uint8_t calc_heater_res(const struct bme680_dev_s *priv)
{
  ...
  int16_t temp = dev.config.target_temp;

  if (temp > 400)
    temp = 400;

  int16_t amb_temp = dev.config.amb_temp;
...
static uint8_t calc_heater_dur(const struct bme680_dev_s *priv)
{
  ...
  uint8_t gas_wait_val = (factor << 6) | heat_dur;
...
static int bme680_set_gas_config(FAR struct bme680_dev_s *priv)
{
  ...
  uint8_t heat_res = calc_heater_res(priv);
  ...
  uint8_t heat_dur = calc_heater_dur(priv);
  ...
  uint8_t run_gas = priv->dev.config.target_temp ? 1 : 0;
  uint8_t regval = (run_gas << 4) | nb_conv;
...
static int bme680_write_config(FAR struct bme680_dev_s *priv)
{
  ...
  uint8_t regval = priv->dev.config.filter_coef << 2;
                             ...
                             uint16_t adc_hum)
{
  ...
  var2 = var1 * ((float)(((float) dev.calib.h2 / 262144.0f)
              * (1.0f + (((float)dev.calib.h4 / 16384.0f)
              * temp_comp) + (((float)dev.calib.h5 / 1048576.0f)
              * temp_comp * temp_comp))));
                                    ...
                                    FAR struct sensor_gas *data)
{
  ...
  uint8_t status = data_regs[0] & BME680_NEW_DATA_MSK;
  ...
  uint32_t adc_temp = (uint32_t) (((uint32_t) data_regs[5] << 12)
...
#ifndef CONFIG_BME680_DISABLE_PRESS_MEAS
  uint32_t adc_press = (uint32_t) (((uint32_t) data_regs[2] << 12)
...
#ifndef CONFIG_BME680_DISABLE_HUM_MEAS
  uint16_t adc_hum = (uint16_t) (((uint32_t) data_regs[8] << 8)
...
#ifndef CONFIG_BME680_DISABLE_GAS_MEAS
  uint16_t adc_gas_res = (uint16_t) ((uint32_t) data_regs[13] << 2
                                    | (((uint32_t) data_regs[14]) >> 6));
  uint8_t gas_range = data_regs[14] & BME680_GAS_RANGE_MSK;

  /* Is measured gas valid? */

  uint8_t gas_valid = data_regs[14] & BME680_GASM_VALID_MSK;
  ...
  uint8_t heat_stab = data_regs[14] & BME680_HEAT_STAB_MSK;
...
static uint16_t bme680_get_tphg_dur(FAR struct bme680_dev_s *priv)
{
  ...
  uint16_t duration = (uint16_t) tph_dur;
                            ...
                            unsigned long arg)
{
  ...
  int ret = bme680_write_config(priv);
...
{
  FAR struct bme680_dev_s *priv = (FAR struct bme680_dev_s *)
        ((uintptr_t)strtoul(argv[1], NULL, 0));
  struct sensor_gas data;
  int ret;
  ...
  while (true)
    {
      ...
      ret = priv->dev.lower.push_event(priv->dev.lower.priv, &data,
